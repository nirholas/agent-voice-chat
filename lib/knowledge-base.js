...
/**
 * Word-based split with overlap. Used as the fallback for any single block
 * (section or paragraph) that is larger than one chunk on its own.
 */
function chunkWords(text, wordsPerChunk, overlapWords) {
...
function chunkByParagraphs(text, chunkSize, overlap) {
  ...
  for (const para of paragraphs) {
    ...
    // A paragraph longer than a whole chunk still has to be split, or it lands
    // in the index as one oversized chunk that no embedding call can accept.
    if (paraWords > wordsPerChunk) {
      ...
      chunks.push(...chunkWords(para.trim(), wordsPerChunk, overlapWords))
      continue
    }
...
function chunkMarkdown(text, chunkSize, overlap) {
  ...
  for (const section of sections) {
    ...
    // A section longer than a whole chunk still has to be split, or it lands in
    // the index as one oversized chunk that no embedding call can accept.
    if (sectionWords > wordsPerChunk) {
      if (group.length > 0) {
        chunks.push(group.join("\n\n"))
        group = []
        groupWordCount = 0
      }
      chunks.push(...chunkWords(section, wordsPerChunk, overlapWords))
      continue
    }
...
class KnowledgeBase {
  ...
  async search(query, opts = {}) {
    ...
    // Keyword scores are normalised against the best keyword hit and capped at
    // 30% of the combined score, so with no embeddings at all every score would
    // collapse onto the threshold itself. Retrieval is unavailable rather than
    // arbitrary: report nothing until the chunks have been embedded.
...
module.exports = { KnowledgeBase, splitSentences, combineSentencesIntoChunks, chunkWords, chunkByParagraphs, chunkMarkdown, parseCSV, parseHTML, preprocessText, extractHeaders, tokenize, bm25Score }

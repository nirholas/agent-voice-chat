...
class MemoryStore {
  ...
  async createVersion() {
    ...
    // Date.now() alone collides when two versions are created inside the same
    // millisecond, which silently overwrites the previous backup. Stay one ahead
    // of the newest version already on disk so every backup keeps its own file.
    const latestVersion = this.listVersions()[0] || 0
